...
`
let countries_obj = JSON.parse(countries);
console.log(countries_obj);


function clearDiv() {
	while(two_div.firstChild)
		{
		two_div.removeChild(two_div.firstChild);
		}
}
...
let btn3= document.querySelector('.output_btn')
let later_elem = document.querySelector('.later')
let arrowUp_elem = document.querySelector('#arrow_name')
...
let two_div = document.querySelector('.second_div')
...
function get_Country(){
	...
	const selected_country_population = []; 
    const input_value = prompt.value.trim().toUpperCase()
	clearDiv()
	for(i = 0; i < countries_obj.length; i++)
		
		{
			let country_part = countries_obj[i].name.startsWith(input_value)
			if(country_part){
...
console.log(selected_country_population);
selected_country_population.sort((a,b) => b - a)
// {askjfnkajnjkfweirjfmkfkf}

let country_name_pop;
let country_pop_array = [];
let country_array_population = [];
for (i = 0; i < selected_country.length; i++) {
	country_name_pop = selected_country[i].name

	country_count_pop = selected_country_population[i]
...
function get_Country_Capital(){
	const selected_country = [];
	let selected_country_population = [];
    const input_value = prompt.value.trim().toUpperCase()
	
	clearDiv()
	for(i = 0; i < countries_obj.length; i++)
		{
			let country_part = false;
			if('capital' in countries_obj[i]){
				country_part = countries_obj[i].capital.startsWith(input_value)
			}
			if(country_part){
				selected_country.push(countries_obj[i])
				selected_country_population.push(countries_obj[i].population)
